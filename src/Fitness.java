    ...
    public int getFitness(Chromozome ch) {
        ...
        for (int y = 0; y < width; y++) {
            for (int x = 0; x < width; x++) {
                ...
                // chyba
                int errorRed = clrChromozom.getRed() - clrObrazek.getRed();
                int errorGreen = clrChromozom.getGreen() - clrObrazek.getGreen();
                int errorBlue = clrChromozom.getBlue() - clrObrazek.getBlue();
                error = (errorRed * errorRed) + (errorGreen * errorGreen) + (errorBlue * errorBlue);
            }
        }
        return error;
    }
